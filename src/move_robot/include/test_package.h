...
void test_package::State_Machine(int &state)
{
	...
	case P_STATE_MOVE:
		...
		if (now_path_index > 0)
		{
			...
			else if (previous_type == MISSON_turn_left || previous_type == MISSON_turn_right || previous_type == MISSON_turn_back_left || previous_type == MISSON_turn_back_right)
			{
...
bool test_package::Tracking_Trajectory(int &subpath_index, bool isReSet)
{
	...
	if (!isReSet)
	{
		...
		//路線很短的情況
		//再剩下30%路徑時開始減速
		int speed_down_count = (int)(Path_size * 0.7);
		int speed_down_road = Path_size - speed_down_count;

		//===前85% 的導航====
		int First_tracking = Path_size * 0.7;
		//std::cout<<"AA"<<std::endl;
		...
		if (!back_trajectory)
		{
			...
			sendreceive.Package_Diff_encoder(V_rv, W_rw, command);
		}
		else
		{
			// sendreceive.Package_testWheel_encoder(-1 * V_rv, 0, W_rw, 0, command);
...
{ //==============change=============
	...
	if (btn_id == PUSE_BUTTON_A)
	{
		...
		sendreceive.Package_Diff_encoder(V, W_rw, command);
	...
	else if (btn_id == PUSE_BUTTON_X)
	{
		ROS_INFO("turn_left");
		std::vector<unsigned char> command;
		float turn_left_w = 0.5;
		//sendreceive.Package_testWheel_encoder(0, 0, turn_left_w, 0, command);
		sendreceive.Package_Diff_encoder(0, turn_left_w, command);
		SendPackage(command);
	}
	else if (btn_id == PUSE_BUTTON_Y)
	{
		ROS_INFO("turn_right");
		std::vector<unsigned char> command;
		float turn_right_w = -0.5;
		//sendreceive.Package_testWheel_encoder(0, 0, turn_right_w, 0, command);
		sendreceive.Package_Diff_encoder(0, turn_right_w, command);
		SendPackage(command);
	}
